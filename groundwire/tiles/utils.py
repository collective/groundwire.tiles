...
def resolve(url):
    ...
    resolved, can_edit = resolveResource(url)
    return html.fromstring(resolved).getroottree(), can_edit
...
def renderTiles(request, tree):
    ...
    for tileId, tileHref, tileNode in tiles:
        ...
        tileTree, can_edit = resolve(tileHref)
        ...
        if tileTree is not None:
            ...
            if tileNode is not None:
                ...
                # set tile-editable class as appropriate
                if can_edit:
                    css_class = tileNode.attrib.get('class', '') + ' tile-editable'
                    tileNode.attrib['class'] = css_class
